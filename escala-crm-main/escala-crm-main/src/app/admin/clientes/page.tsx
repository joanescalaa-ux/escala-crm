      ...
      <div style={{ backgroundColor: '#fff', borderRadius: 16, padding: 32, width: '90%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <button type="submit" disabled={loading} style={{ flex: 1, height: 40, backgroundColor: '#0A0A0A', color: '#F8F284', border: 'none', borderRadius: 8, cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, fontWeight: 600 }}>
    ...
    <div className="p-6">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <button onClick={() => setModal({ open: true, item: null })}
          style={{ height: 36, backgroundColor: '#0A0A0A', color: '#F8F284', border: 'none', borderRadius: 8, padding: '0 16px', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {!loading && filtered.length === 0 && (
              <tr>
                <td colSpan={7} style={{ textAlign: 'center', padding: 64 }}>
                  ...
                  <button onClick={() => setModal({ open: true, item: null })}
                    style={{ height: 36, backgroundColor: '#0A0A0A', color: '#F8F284', border: 'none', borderRadius: 8, padding: '0 20px', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}>
                    + Añadir Cliente
                  </button>
                </td>
              </tr>
                      ...
                      <div style={{
                        ...
                        <button onClick={async () => {
                          ...
                          const res = await fetch('/api/auth/invite', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ email: c.email, cliente_id: c.id }),
                          })
                          const data = await res.json()
                          if (data.codigo) {
                            await navigator.clipboard.writeText(data.codigo).catch(() => {})
                            alert(`✅ Código generado y copiado:\n\n${data.codigo}\n\nEnvíaselo a ${c.email}`)
                          }
                        }}
                          style={{ width: '100%', padding: '8px 12px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8, color: '#0A0A0A', borderRadius: 4, fontFamily: 'inherit', fontWeight: 600 }}>
                          🔑 Generar código de acceso
                        </button>
                        <button onClick={async () => {
                          setMenuOpen(null)
                          const res = await fetch(`/api/programa/fases?clienteId=${c.id}`)
                          const data = await res.json()
                          const actual = data.desbloqueadas || [1]
                          const opciones = [1,2,3,4,5].map(f => `Fase ${f}: ${actual.includes(f) ? '✅ desbloqueada' : '🔒 bloqueada'}`).join('\n')
                          const input = prompt(`Fases de ${c.nombre}:\n${opciones}\n\n¿Qué fase quieres cambiar? (1-5)`)
                          if (!input) return
                          const fase = parseInt(input)
                          if (isNaN(fase) || fase < 1 || fase > 5) return alert('Fase inválida')
                          const desbloquear = !actual.includes(fase)
                          await fetch('/api/programa/fases', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ cliente_id: c.id, fase, desbloquear }),
                          })
                          alert(`Fase ${fase} ${desbloquear ? 'desbloqueada' : 'bloqueada'} para ${c.nombre}`)
                        }}
                          style={{ width: '100%', padding: '8px 12px', background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8, color: '#0A0A0A', borderRadius: 4, fontFamily: 'inherit' }}>
                          🗺️ Gestionar fases
                        </button>
